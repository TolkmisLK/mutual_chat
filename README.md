...
**开发预览，尚未达到正式上线验收。** 当前包含独立 Web 应用、可嵌入的 ES 模块、会话列表、文字消息、手动接受邀请与创建私密加密会话。桌面/移动原生打包、服务端部署、密钥恢复、设备验证、附件和真实多用户端到端测试仍待完成。
...
- 当前独立应用每次登录创建新设备，使用独立 IndexedDB 密钥库；SDK 默认数据库未由本应用加设本地口令加密。
- 设备验证、跨签名、恢复密钥、备份恢复与会话恢复 UI **尚未实现**。因此不能将当前版本用于依赖历史密文可恢复性的正式通信。
- 只允许一个独立窗口持有 SDK；嵌入场景的 client 生命周期由宿主管理，不允许多个 client 同时写同一个加密数据库。
- 消息正文按纯文本渲染，不执行消息中的 HTML。

## 验证与下一步

本地 5 项核心测试与两种生产构建已通过；测试使用受控 SDK 替身，未宣称验证了真正的加密互通。后续验收见 [ROADMAP.md](docs/ROADMAP.md)。
