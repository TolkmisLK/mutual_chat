...
PostgreSQL 开发部署与备份恢复演练见 [POSTGRES.md](docs/POSTGRES.md)；仅监听本机，不等于公开上线。

可独立运行，也可嵌入其他产品的聊天界面。参考常见即时聊天软件的会话列表和消息交互，使用 Matrix 协议连接聊天服务。

**开发预览，尚未达到正式上线验收。** 当前包含独立 Web 应用、可嵌入 ES 模块与 Windows 桌面预览包，以及文字消息、邀请、私密加密会话和历史分页。本机 Matrix 服务、双用户加密互通、已有备份的新设备历史恢复、同帐号设备数字核对、Windows 实际窗口启动及 Linux 桌面进程加密互通已通过相应测试；公开服务、Windows 真机与原生加密验收、移动客户端、首次备份、联系人身份验证和附件仍待完成。
...
「搜索已加载消息」只在本机查找当前会话已加载的消息，可逐条跳转，不把搜索词发送给服务器，也不会自动标为已读。它不搜索尚未加载的历史或附件，详见 [LOCAL-SEARCH.md](docs/LOCAL-SEARCH.md)。
...
- 独立应用中的「核对设备」支持同帐号另一设备的 SAS 数字核对；需通过面对面或独立可信渠道比较双方全部数字，不自动确认。它不等于联系人身份验证、跨签名设置或首次备份。详见 [设备数字核对](docs/DEVICE-VERIFICATION.md)。
- 跨签名和首次远程备份设置 **尚未实现**。没有已保存的恢复密钥或有效备份时，清理浏览器数据、退出未备份设备仍可能失去历史密钥，不能将预览版当成完整通信产品。
...
最近同帐号设备核对候选通过 57 项单元测试、三种 Web/模块构建和 13 项 Chromium 场景；Windows 打包启动、Linux 桌面加密互通/重启恢复及 PostgreSQL 恢复也再次通过。实际手机视口核对界面已检查，但不等于手机真机验收。结果与平台边界见 [VALIDATION.md](docs/VALIDATION.md)，后续验收见 [ROADMAP.md](docs/ROADMAP.md)。
