...
export function mountChat(container, { client, session: providedSession }) {
  ...
  const q = s => root.querySelector(s); let selected; let sending = false; let stopped = false;
  ...
  q('form').onsubmit = async e => {
    e.preventDefault(); const input = q('textarea'); const text = input.value; const target = selected;
    sending = true; render();
    try { await session.sendText(target, text); if (input.value === text) input.value = ''; status(''); }
  ...
  return { session, unmount() { stopped = true; unsubscribe(); if (!providedSession) session.dispose(); host.remove(); } };
}
